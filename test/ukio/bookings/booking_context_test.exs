defmodule Ukio.Bookings.BookingContextTest do
    use Ukio.DataCase
    ...
    describe "bookings" do
        ...
        test "list_bookings/0 returns all bookings" do
            ...
            assert BookingContext.list_bookings() == [booking]
        end

        test "get_booking!/1 returns the booking with given id" do
            booking = booking_fixture()
            assert BookingContext.get_booking!(booking.id) == booking
        end
        ...
        test "create_booking/1 with valid data creates a booking", %{apartment: apartment} do
            ...
            assert {:ok, %Booking{} = booking} = BookingContext.create_booking(valid_attrs)
        ...
        test "create_booking/1 with invalid data returns error changeset" do
            assert {:error, %Ecto.Changeset{}} = BookingContext.create_booking(@invalid_attrs)
        end
        ...
        test "update_booking/2 with valid data updates the booking" do
            ...
            assert {:ok, %Booking{} = booking} = BookingContext.update_booking(booking, update_attrs)
        ...
        test "update_booking/2 with invalid data returns error changeset" do
            booking = booking_fixture()
            assert {:error, %Ecto.Changeset{}} = BookingContext.update_booking(booking, @invalid_attrs)
            assert booking == BookingContext.get_booking!(booking.id)
        end

        test "delete_booking/1 deletes the booking" do
            booking = booking_fixture()
            assert {:ok, %Booking{}} = BookingContext.delete_booking(booking)
            assert_raise Ecto.NoResultsError, fn -> BookingContext.get_booking!(booking.id) end
        end

        test "change_booking/1 returns a booking changeset" do
            booking = booking_fixture()
            assert %Ecto.Changeset{} = BookingContext.change_booking(booking)
        end
    end
end